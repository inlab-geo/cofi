...
def test_run():
    ...
    y = forward_func(np.array([0,0,1]), x) + np.random.normal(20)
    ...
    plt.scatter(x, forward_func(res["model"], x), color="orange")
    plt.show()
    print(res)
    assert res["success"]
    assert pytest.approx(res["model"], abs=0.1) == np.array([0,1,1])