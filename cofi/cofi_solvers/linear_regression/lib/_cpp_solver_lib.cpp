...
static double **malloc_mat(int rows, int cols) {
    double **mat = (double **) malloc(rows * sizeof(*mat));
    for (int i = 0; i < rows; i++)
        mat[i] = (double *) malloc(cols * sizeof(*(mat[i])));
    return mat;
}
...
static double get_determinant(int n, std::vector< std::vector<double> > mat) {
    ...
    double det = 0;
    std::vector< std::vector<double> > tmp;    // tmp: nxn
    int sign = 1;
    for (int f = 0; f < n; f++) {
...
static std::vector< std::vector<double> > inverse(int n, std::vector< std::vector<double> > mat) {
    ...
    std::vector< std::vector<double> > tmp; // = malloc_mat(n, n);
    std::vector< std::vector<double> > fac; // = malloc_mat(n, n);
    int p, q, a, b, i, j;
    for (q = 0; q < n; q++) {
        for (p = 0; p < n; p++) {
            a = 0;
