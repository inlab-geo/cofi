...
def test_xrt_obj_init(data_path):
    ...
    with pytest.raises(ValueError, match=r".*full data.*including data_attns and data_paths.*"):
        XRayTomographyObjective(data_attns=d)
    with pytest.raises(ValueError, match=r".*dimensions between data_attns and data_paths don't.*"):
...
def test_xrt_misfit(example_model_paths):
    ...
    xrt_obj = XRayTomographyObjective(data_attns=attenuations, data_paths=paths, extent=(0,1,0,1))
