...
class LpNormRegularization(BaseRegularization):
    ...
    ----------
    p : Number or str
        order value passed directly to :func:`scipy.linalg.norm`, default to 2
    ...
    def _generate_matrix(self):
        ...
        if isinstance(self._weighting_matrix, str) or self._weighting_matrix is None:
            ...
            if _reg_type == "damping" or _reg_type is None:          # 0th order difference
                ...
                if np.size(self.model_shape) == 1:              # 1D model
                ...
                elif np.size(self.model_shape) == 2 and np.ndim(self.model_shape) == 1:     # 2D model
                    ...
                    self._weighting_matrix = np.vstack((matx.toarray(), maty.toarray()))  # combine above
                else:
                    raise NotImplementedError(
